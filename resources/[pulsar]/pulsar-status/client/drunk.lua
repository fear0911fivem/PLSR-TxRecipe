                        ...
                        if lastBlackout <= 0 then
                            ...
                            if lastBlackout <= GetGameTimer() then
                                ...
                                if veh and GetPedInVehicleSeat(veh, -1) == LocalPlayer.state.ped then
                                    TaskVehicleTempAction(LocalPlayer.state.ped, veh,
                                        _drunkVehicleActions[math.random(#_drunkVehicleActions)], 5000)
                                end
...
function IsDrinkingAlcohol()
    local doingAnim = exports['pulsar-animations']:EmotesGet()
...
function RegisterDrunkCallbacks()
    exports["pulsar-core"]:RegisterClientCallback('Status:DrinkAlcohol', function(data, cb)
        if IsDrinkingAlcohol() then
            exports["pulsar-hud"]:Notification("error", 'Already Drinking!')
            return cb(false)
        end

        local alcohol = _alcoholConfig[data]
        if alcohol then
            exports['pulsar-animations']:EmotesPlay(alcohol.anim, false, alcohol.initialTime, true)
            exports['pulsar-hud']:Progress({
                ...
                if not cancelled then
                    ...
                    exports['pulsar-animations']:EmotesPlay(alcohol.anim, false, false, false)

                    local drunkLevel = exports['pulsar-status']:GetSingle("PLAYER_DRUNK").value
                    if drunkLevel < alcohol.drunkBaseline then
                        exports['pulsar-status']:SetSingle("PLAYER_DRUNK", alcohol.drunkBaseline)
                    end

                    exports['pulsar-status']:Add("PLAYER_THIRST", alcohol.thirst)
                    exports['pulsar-status']:Remove("PLAYER_STRESS", alcohol.stressRelief, true)

                    local endTime = GetGameTimer() + alcohol.maxTime

                    CreateThread(function()
                        local tick = 3
                        while LocalPlayer.state.loggedIn and exports['pulsar-animations']:EmotesGet() == alcohol.anim do
                            if tick >= 5 then
                                exports['pulsar-status']:Add("PLAYER_DRUNK", alcohol.drunkPerTick)
