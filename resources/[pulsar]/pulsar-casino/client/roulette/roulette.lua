...
AddEventHandler("Casino:Client:Startup", function()
    ...
    exports["pulsar-kbs"]:Add("casino_camera", "C", "keyboard", "Casino - Change Roulette Camera", function()
...
AddEventHandler("Casino:Client:JoinRoulette", function(data)
    ...
    if chairData then
        exports["pulsar-core"]:ServerCallback("Casino:JoinRoulette", { table = data.table, chair = chairData.chairId },
        ...
        exports["pulsar-hud"]:Notification("error", "Too Far From Chair")
    end
end)

function LeaveRoulette(skipAnim)
    exports["pulsar-core"]:ServerCallback("Casino:LeaveRoulette", {}, function(success)
        if success then
...
function RoulettePlaceChips(betType)
    ...
    if betAmount then
        exports["pulsar-core"]:ServerCallback("Casino:BetRoulette", {
            amount = betAmount,
            betId = betType,
        }, function(success)
            if success then
...
function ShowRouletteGameStateUI(state)
    if state.Started then
        exports['pulsar-hud']:InfoOverlayShow("Roulette", "The Wheel is Spinning")
    else
        local myBets = 0
        local myBalance = formatNumberToCurrency(math.floor(exports['pulsar-casino']:ChipsGet()))
        ...
        exports['pulsar-hud']:InfoOverlayShow(string.format("Roulette%s", showStartTime),
