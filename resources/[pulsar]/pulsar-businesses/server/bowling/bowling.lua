...
AddEventHandler('Businesses:Server:Startup', function()
    ...
    exports["pulsar-core"]:RegisterServerCallback('Bowling:StartGame', function(source, data, cb)
    ...
    exports["pulsar-core"]:RegisterServerCallback('Bowling:JoinGame', function(source, data, cb)
    ...
    exports["pulsar-core"]:RegisterServerCallback('Bowling:EndGame', function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
    ...
    exports["pulsar-core"]:RegisterServerCallback('Bowling:UpdateTVLink', function(source, data, cb)
    ...
    exports["pulsar-core"]:RegisterServerCallback('Bowling:ResetAll', function(source, data, cb)
        if Player(source).state.onDuty == 'bowling' then
    ...
    exports["pulsar-core"]:RegisterServerCallback('Bowling:ClearPins', function(source, data, cb)
        if Player(source).state.onDuty == 'bowling' then
            for k, v in ipairs(GetAllObjects()) do
                local model = GetEntityModel(v)
                if model == -1501785249 or model == -563331074 and #(GetEntityCoords(v) - vector3(738.970, -775.074, 26.446)) <= 100.0 then
...
RegisterServerEvent('Bowling:Server:StartBowling', function(alleyId)
    ...
    if char and alleyId then
        ...
        if alley and alley.active and mySID == alley.currentPlayer then
            exports["pulsar-core"]:ClientCallback(source, 'Bowling:DoBowl', alleyId, function(res)
