...
AddEventHandler("Businesses:Client:Startup", function()
	...
	exports['pulsar-polyzone']:CreateBox("uwu_cafe_door", vector3(-581.05, -1068.09, 22.34), 3.0, 1.6, {
		heading = 0,
		--debugPoly=true,
		minZ = 21.34,
		maxZ = 24.54,
...
AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
	if id == "uwu_cafe_door" then
		...
		if h > 310 or h < 50 then
			exports["pulsar-sounds"]:PlayDistance(15, "bell.ogg", 0.3)
		end
	end
end)
...
AddEventHandler("Businesses:Client:PetCat", function(data)
	if data.entity == nil then
		exports["pulsar-hud"]:Notification("error", "This is not a cat!")
		return
	end
	TaskTurnPedToFaceEntity(LocalPlayer.state.ped, data.entity, 1000)
	Wait(1000)
	exports["pulsar-core"]:ServerCallback("Businesses:Server:PetCat", data, function(success, newState)
		if success then
			exports['pulsar-animations']:EmotesPlay("petting", false, 3000, true)
			Wait(3000)
			exports["pulsar-hud"]:Notification("success", "Feeling less stressful.")
		end
	end)
end)
...
RegisterNetEvent("Inventory:Client:Collectables:UseItem", function(itemName)
	if itemName == "uwu_prize_b1" then -- brown
		exports['pulsar-animations']:EmotesPlay("uwubrownplush", false, false, false)
	end

	if itemName == "uwu_prize_b2" then -- orange
		exports['pulsar-animations']:EmotesPlay("uwuorangeplush", false, false, false)
	end
	...
	if itemName == "uwu_prize_b4" then -- green
		exports['pulsar-animations']:EmotesPlay("uwugreenplush", false, false, false)
	end

	if itemName == "uwu_prize_b5" or itemName == "uwu_prize_b7" then -- pink
		exports['pulsar-animations']:EmotesPlay("uwupurpleplush", false, false, false)
	end

	if itemName == "uwu_prize_b6" then -- yellow
		exports['pulsar-animations']:EmotesPlay("uwuyellowplush", false, false, false)
	end

	if itemName == "uwu_prize_b8" then -- blue
		exports['pulsar-animations']:EmotesPlay("uwublueplush", false, false, false)
	end

	if itemName == "uwu_prize_b9" then -- grey
		exports['pulsar-animations']:EmotesPlay("uwuwasabiplush", false, false, false)
	end

	if itemName == "uwu_prize_b10" then -- sky blue
		exports['pulsar-animations']:EmotesPlay("uwuprincessplush", false, false, false)
	end
end)
