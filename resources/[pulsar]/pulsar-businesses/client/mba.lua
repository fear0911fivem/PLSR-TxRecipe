...
AddEventHandler("Businesses:Client:MBA:StartChangeInterior", function()
	...
	exports['pulsar-hud']:InputShow(string.format("Change Event Floor - Current: %s", _eventNames[current]),
		"Match Configuration", {
			{
				id = "interior",
				type = "select",
...
AddEventHandler("Businesses:Client:MBA:ChangeInterior", function(values)
	if values?.interior then
		exports["pulsar-core"]:ServerCallback("MBA:ChangeInterior", values.interior, function(success)
			if success then
				exports["pulsar-hud"]:Notification("success", "Updated")
			else
				exports["pulsar-hud"]:Notification("error", "Failed to Change Event Floor")
			end
		end)
	end
end)
