...
AddEventHandler('Businesses:Client:Startup', function()
    exports['pulsar-polyzone']:CreatePoly('vu_dancers', {
    ...
    exports['pulsar-polyzone']:CreatePoly('vu_makeitrain', {
    ...
    exports['pulsar-hud']:InteractionRegisterMenu("vu_stripper_pole", "Vanilla Unicorn Dancers", "party-horn", function()
        if exports['pulsar-polyzone']:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_dancers') and LocalPlayer.state.onDuty == 'unicorn' and exports['pulsar-jobs']:HasPermissionInJob('unicorn', 'STRIP_POLE') then
            local subMenu = {}
        ...
        return exports['pulsar-polyzone']:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_dancers') and LocalPlayer.state.onDuty == 'unicorn'
    end)

    exports['pulsar-hud']:InteractionRegisterMenu("vu_makeitrain", 'Make It Rain', "money-bill-1-wave", function()
        if not _makingItRain and exports['pulsar-polyzone']:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_makeitrain') then
            local makeItRain = {
                {
                    type = 'cash',
                    text = '$100 Cash',
        ...
        return exports['pulsar-polyzone']:IsCoordsInZone(GetEntityCoords(LocalPlayer.state.ped), 'vu_makeitrain') and GetNearbyStripper()
    end)
end)
...
function MakeItRainBitch(targetSource, cashType, time)
    ...
    CreateThread(function()
        ...
        do
            local p = promise.new()
            exports["pulsar-core"]:ServerCallback('VU:MakeItRain', {
                target = targetSource,
                type = cashType,
            }, function(success, cd)
                if not success then
