...
local url = 'nui://pulsar-businesses/dui/bowling/index.html'
local scale = 0.08
...
function GetBowlingTVLink()
    ...
    exports['pulsar-hud']:InputShow('Bowling', 'URL', {
        {
            id = 'name',
            type = 'text',
            options = {
...
AddEventHandler('Bowling:Client:SetTV', function()
    ...
    exports["pulsar-core"]:ServerCallback('Bowling:UpdateTVLink', tvLink, function(success)
        if success then
            SendBowlingNotification('Updated Link!')
        else
            exports["pulsar-hud"]:Notification("error", 'Unable to Update Link', 5000, 'bowling-ball-pin')
        end
    end)
end)
