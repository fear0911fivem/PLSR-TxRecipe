...
AddEventHandler('Businesses:Client:Startup', function()
    ...
    exports["pulsar-kbs"]:Add('bowling_aim_right', 'RIGHT', 'keyboard', 'Bowling - Aim Right', function()
    ...
    exports["pulsar-kbs"]:Add('bowling_aim_left', 'LEFT', 'keyboard', 'Bowling - Aim Left', function()
        if awaitingBowl then
            local heading = GetEntityHeading(LocalPlayer.state.ped) + 0.7
    ...
    exports["pulsar-core"]:RegisterClientCallback('Bowling:DoBowl', function(data, cb)
...
function SendBowlingNotification(text, time)
    exports["pulsar-hud"]:NotifCustom(text, time or 5000, 'bowling-ball-pin', {
        alert = {
            background = "#C05097",
        },
        progress = {
...
function StartBowlingShit(alleyId, isSecondTry, currentPinsDown, currentHitPins)
    ...
    pressedBowl = false
    exports['pulsar-hud']:ActionShow('bowling',
    ...
    exports['pulsar-hud']:ActionHide('bowling')
    awaitingBowl = false
            ...
            CreateThread(function()
                ...
                for k, v in ipairs(pins) do
                    ...
                    if not (entRotation.x <= 10.0 and entRotation.x >= -10.0 and entRotation.y <= 10.0 and entRotation.y >= -10.0 and entRotation.z <= 10.0 and entRotation.z >= -10.0) then
                        exports["pulsar-sounds"]:PlayLocation(alleyData.endZone, 20.0, "bowling.ogg", 0.3)
                        break
                    end
                end
            end)
...
function GetBowlingNickName()
    ...
    exports['pulsar-hud']:InputShow('Bowling', 'Name', {
        {
            id = 'name',
            type = 'text',
            options = {
...
AddEventHandler('Bowling:Client:StartGame', function(alleyId)
    ...
    if nickName and string.len(nickName) >= 3 then
        exports["pulsar-core"]:ServerCallback('Bowling:StartGame', {
            alleyId = alleyId,
            nickName = nickName,
        }, function(success)
            if success then
                ...
                exports["pulsar-hud"]:Notification("error", 'Couldn\'t Start Game', 5000, 'bowling-ball-pin')
            end
        end)
    else
        exports["pulsar-hud"]:Notification("error", 'Please provide a name longer than 2 letters', 5000,
            'bowling-ball-pin')
    end
end)
...
AddEventHandler('Bowling:Client:JoinGame', function(alleyId)
    local nickName = GetBowlingNickName()
    if nickName and string.len(nickName) >= 3 then
        exports["pulsar-core"]:ServerCallback('Bowling:JoinGame', {
            alleyId = alleyId,
            nickName = nickName,
        }, function(success)
            if success then
                ...
                exports["pulsar-hud"]:Notification("error", 'Couldn\'t Join Game', 5000, 'bowling-ball-pin')
            end
        end)
    else
        exports["pulsar-hud"]:Notification("error", 'Please provide a name longer than 2 letters', 5000,
...
AddEventHandler('Polyzone:Enter', function(id, testedPoint, insideZones, data)
    if data?.bowling_start then
        ...
        exports['pulsar-hud']:ActionShow('bowling', '{keybind}primary_action{/keybind} Start Bowling')
...
AddEventHandler('Keybinds:Client:KeyUp:primary_action', function()
    if insideBowlingStart and not awaitingBowl then
        ...
        if alley and not alley.finished and alley.currentPlayer == LocalPlayer.state.Character:GetData('SID') then
            ...
            exports["pulsar-hud"]:Notification("error", 'Not Your Turn', 5000, 'bowling-ball-pin')
        end
    end
end)

AddEventHandler('Bowling:Client:EndGame', function(alleyId)
    exports["pulsar-core"]:ServerCallback('Bowling:EndGame', {
        alleyId = alleyId,
    }, function(success)
        if success then
            SendBowlingNotification('Ended a Game of Bowling')
        else
            exports["pulsar-hud"]:Notification("error", 'Couldn\'t End Game', 5000, 'bowling-ball-pin')
        end
    end)
end)

AddEventHandler('Bowling:Client:ResetAll', function()
    exports["pulsar-core"]:ServerCallback('Bowling:ResetAll', {}, function(success)
        if success then
            SendBowlingNotification('Reset All')
        else
            exports["pulsar-hud"]:Notification("error", 'Couldn\'t Reset', 5000, 'bowling-ball-pin')
        end
    end)
end)

AddEventHandler('Bowling:Client:ClearPins', function()
    exports["pulsar-core"]:ServerCallback('Bowling:ClearPins', {}, function(success)
        if success then
            SendBowlingNotification('Cleared Pins')
        else
            exports["pulsar-hud"]:Notification("error", 'Couldn\'t Clear Pins', 5000, 'bowling-ball-pin')
        end
    end)
end)
