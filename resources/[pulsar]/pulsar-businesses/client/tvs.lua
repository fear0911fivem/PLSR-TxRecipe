...
local createdDUIs = {}
local url = 'nui://pulsar-businesses/dui/tvs/index.html'
local scale = 0.08
...
function GetNewTVLink()
    ...
    exports['pulsar-hud']:InputShow('TVs', 'URL', {
        {
            id = 'name',
            type = 'text',
            options = {
...
AddEventHandler('TVs:Client:SetLink', function(data)
    ...
    exports["pulsar-core"]:ServerCallback('TVs:UpdateTVLink', {
        tv = data,
        link = tvLink
    }, function(success)
        if success then
            exports["pulsar-hud"]:Notification("success", 'Updated Link!', 5000, 'tv')
        else
            exports["pulsar-hud"]:Notification("error", 'Unable to Update Link', 5000, 'tv')
        end
    end)
end)
