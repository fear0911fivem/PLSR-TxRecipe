	...
	if inHeli and camHelis[GetEntityModel(inHeli)] then
		...
		then
			if _hasRappel == false then
				_hasRappel = true
				exports["pulsar-hud"]:Notification("info", "Prepping Rappel")
				PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
				TaskRappelFromHeli(LocalPlayer.state.ped, 1)
				Wait(10000)
				_hasRappel = false
			else
				exports["pulsar-hud"]:Notification("error", "Rappel is already being initiated!")
			end
		else
			exports["pulsar-hud"]:Notification("info", "Cannot rappel from this seat")
