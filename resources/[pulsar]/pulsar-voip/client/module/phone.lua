RegisterNetEvent('VOIP:Phone:Client:SyncCallData', function(callTable, channel)
	...
	for tgt, enabled in pairs(CALL_DATA) do
		if tgt ~= playerServerId then
			exports["pulsar-voip"]:ToggleVoice(tgt, enabled, 'phone')
		end
	end
end)
	...
	if plySource == PLAYER_SERVER_ID then
		for tgt, enabled in pairs(CALL_DATA) do
			if tgt ~= PLAYER_SERVER_ID then
				exports["pulsar-voip"]:ToggleVoice(tgt, false, 'phone')
			end
		end
		CALL_DATA = {}
		MumbleClearVoiceTargetPlayers(1)
		exports["pulsar-voip"]:SetPlayerTargets(RADIO_TALKING and RADIO_DATA or {}, CALL_DATA)
	else
		CALL_DATA[plySource] = nil
		exports["pulsar-voip"]:ToggleVoice(plySource, false, 'phone')
		MumbleClearVoiceTargetPlayers(1)
		exports["pulsar-voip"]:SetPlayerTargets(RADIO_TALKING and RADIO_DATA or {}, CALL_DATA)
	end
...
RegisterNetEvent('VOIP:Phone:Client:SetPlayerTalkState', function(targetSource, enabled)
	if targetSource ~= PLAYER_SERVER_ID then
		...
		exports["pulsar-voip"]:ToggleVoice(targetSource, enabled, 'phone')
	end
end)
