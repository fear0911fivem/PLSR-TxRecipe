...
AddEventHandler('onResourceStart', function(resource)
  if resource == GetCurrentResourceName() then
    Wait(1000)
    RegisterCallbacks()
    exports['pulsar-core']:VersionCheck('PulsarFW/pulsar-dj')
  end
end)

function RegisterCallbacks()
  exports["pulsar-core"]:RegisterServerCallback("pulsar-dj:server:songInfo", function(source, data, cb)
    cb(previousSongs)
  end)

  exports["pulsar-core"]:RegisterServerCallback("pulsar-dj:server:playMusic", function(source, data, cb)
    local src = source
  ...
  exports["pulsar-core"]:RegisterServerCallback("pulsar-dj:server:changeVolume", function(source, data, cb)
    local src = source
    local zoneNum = data.zoneNum
  ...
  exports["pulsar-core"]:RegisterServerCallback("pulsar-dj:server:stopMusic", function(source, data, cb)
    local src = source
    ...
    TriggerClientEvent("pulsar-dj:client:playMusic", src, { zone = data.zoneNum })
  end)

  exports["pulsar-core"]:RegisterServerCallback("pulsar-dj:server:pauseMusic", function(source, data, cb)
    local src = source
    local zoneLabel = data.zoneName
    if Config.Locations[data.zoneNum].playing then
      Config.Locations[data.zoneNum].playing = nil
  ...
  exports["pulsar-core"]:RegisterServerCallback("pulsar-dj:server:resumeMusic", function(source, data, cb)
    local src = source
    local zoneLabel = data.zoneName
...
-- -- This is for that
-- RegisterNetEvent("pulsar-dj:server:DestoryAll", function()
...
-- 	TriggerEvent("pulsar-dj:server:DestoryAll")
-- end)
