				...
				if IsPedShooting(LocalPlayer.state.ped) and not _ignored[cw] then
					if LocalPlayer.state.onDuty ~= "police" then
						...
						if veh ~= 0 then
							...
							exports['pulsar-mdt']:EmergencyAlertsCreateIfReported(500.0, "shotsfiredvehicle", true, {
								icon = "car",
								details = vehName,
								vehicleColor = {
									r = r,
						...
						elseif IsPedCurrentWeaponSilenced(LocalPlayer.state.ped) then
							exports['pulsar-mdt']:EmergencyAlertsCreateIfReported(10.0, "shotsfired", true)
						else
							exports['pulsar-mdt']:EmergencyAlertsCreateIfReported(900.0, "shotsfired", true)
						end
					end
					Wait(60000)
				end
	...
	CreateThread(function()
		while LocalPlayer.state.loggedIn do
			if not LocalPlayer.state.isDead then
				...
				then
					exports['pulsar-status']:Add("PLAYER_STRESS", 1, false, true)
					Wait(40000)
				end
				Wait(100)
			else
