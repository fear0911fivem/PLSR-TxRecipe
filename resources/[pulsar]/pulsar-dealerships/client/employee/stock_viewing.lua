...
function ViewDealershipStocks(dealerId)
    ...
    if dealerData then
        exports["pulsar-core"]:ServerCallback('Dealerships:StockViewing:FetchData', dealerId,
            function(authed, stocks, serverTime, dealerProfitPercent)
                if not authed then
                    exports["pulsar-hud"]:Notification("error", 'You\'re Not Authorized to View Stock', 3500,
                        'car-building')
                    return
                end
                ...
                stockViewing = exports['pulsar-menu']:Create('stockViewing',
                ...
                for _, category in ipairs(orderedCategories) do
                    if stockData.sorted[category] and #stockData.sorted[category] > 0 then
                        stockViewingSub[category] = exports['pulsar-menu']:Create('stockViewingCat-' .. category,
                        ...
                        for k, v in ipairs(stockData.sorted[category]) do
                            stockViewingSub[category].Add:Text(string.format(
                                ...
                                v.price and
                                '$' ..
                                formatNumberToCurrency(math.floor(exports['pulsar-core']:UtilsRound(
