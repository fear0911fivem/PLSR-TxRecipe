...
AddEventHandler("Vehicles:Client:StartUp", function()
	exports["pulsar-core"]:RegisterClientCallback("Vehicles:Keys:GetVehicleToShare", function(data, cb)
		...
		if DoesEntityExist(veh) then
			...
			if
				...
				and exports['pulsar-vehicles']:KeysHas(vehEnt.state.VIN, vehEnt.state.GroupKeys)
			then
	...
	exports["pulsar-kbs"]:Add("vehicle_lock", "l", "keyboard", "Vehicle - Toggle Door Locks", function()
		if not _lockdelay then
			...
			if DoesEntityExist(veh) then
				...
				exports['pulsar-vehicles']:SetLocks(veh)
			end
		end
	end)
end)
	...
	if entityData and DoesEntityExist(entityData.entity) then
		...
		for i = -1, GetVehicleModelNumberOfSeats(vehmodel) do
			if GetPedInVehicleSeat(entityData.entity, i) == 0 then
				...
				Wait(100)
				exports['pulsar-animations']:EmotesPlay("dogsitcar", false, false, false)
				return
			end
		end
	end
...
exports("SetLocks", function(veh, state)
	...
	exports["pulsar-core"]:ServerCallback("Vehicles:ToggleLocks", {
		...
		if success then
			UnlockAnim()
			if newState then
				exports["pulsar-hud"]:Notification("error", "Vehicle Locked")
				exports["pulsar-sounds"]:PlayOne("central-locking.ogg", 0.2)
				DoVehicleLockShit(veh)
			else
				exports["pulsar-hud"]:Notification("success", "Vehicle Unlocked")
				exports["pulsar-sounds"]:PlayOne("central-locking.ogg", 0.2)
...
AddEventHandler("Vehicles:Client:ToggleLocks", function(entityData)
	...
	exports['pulsar-vehicles']:SetLocks(entityData.entity)
end)
