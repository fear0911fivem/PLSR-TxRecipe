...
AddEventHandler('Vehicles:Client:StartUp', function()
	exports['pulsar-pedinteraction']:Add('veh_impound', `ig_floyd`, vector3(-193.282, -1162.433, 22.7), 270.4, 10.0, {
		{
			icon = "fas fa-truck",
...
AddEventHandler('Vehicles:Client:CharacterLogin', function()
	if _vehicleImpound then
		exports["pulsar-blips"]:Add('veh_impound', "Impound Yard | Tow Job", _vehicleImpound.coords, 317, 16, 0.6)
	end
end)
...
AddEventHandler('Vehicles:Client:RequestTow', function(entityData)
	if DoesEntityExist(entityData.entity) and IsVehicleEmpty(entityData.entity) then
		...
		if vState and vState.VIN then
			...
			local canRegularImpound = exports['pulsar-jobs']:HasPermission(_impoundConfig.RequiredPermission)
			local canPoliceImpound = exports['pulsar-jobs']:HasPermission(_impoundConfig.Police
				.RequiredPermission)
		...
		if #(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0 and IsVehicleEmpty(attemptingImpoundOnVehicle) then
			exports['pulsar-hud']:ProgressWithTickEvent({
				...
				if not cancelled and DoesEntityExist(attemptingImpoundOnVehicle) and (#(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0) and IsVehicleEmpty(attemptingImpoundOnVehicle) then
					exports["pulsar-core"]:ServerCallback("Vehicles:Impound:TagVehicle", {
						...
						exports["pulsar-hud"]:Notification("success", 'Tow Requested')
					end)
				else
					exports["pulsar-hud"]:Notification("error", 'Tow Request Failed')
				end
			end)
		else
			exports["pulsar-hud"]:Notification("error", 'Cannot Request Tow On That Vehicle Anymore...')
		end
	end
end)
...
AddEventHandler('Vehicles:Client:Impound', function(data)
	if attemptingImpoundOnVehicle and DoesEntityExist(attemptingImpoundOnVehicle) then
		if #(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0 and IsVehicleEmpty(attemptingImpoundOnVehicle) then
			exports['pulsar-hud']:ProgressWithTickEvent({
				...
				if not cancelled and DoesEntityExist(attemptingImpoundOnVehicle) and (#(GetEntityCoords(attemptingImpoundOnVehicle) - GetEntityCoords(GLOBAL_PED)) <= 10.0) and IsVehicleEmpty(attemptingImpoundOnVehicle) then
					exports["pulsar-core"]:ServerCallback('Vehicles:Impound', {
						...
						if success then
							exports["pulsar-hud"]:Notification("success", 'Vehicle Impounded')
						else
			...
			exports["pulsar-hud"]:Notification("error", 'Cannot Impound That Vehicle Anymore...')
		end
	end
end)

AddEventHandler('Vehicles:Client:TowReleaseMenu', function()
	exports["pulsar-core"]:ServerCallback('Vehicles:GetVehiclesInImpound', {}, function(vehicleData, time)
		if vehicleData and #vehicleData > 0 then
			...
			exports['pulsar-hud']:ListMenuShow(impoundMenu)
		else
			exports["pulsar-hud"]:Notification("info", 'None of Your Vehicles Are Impounded')
		end
	end)
end)
...
AddEventHandler('Vehicles:Client:TowRequestRelease', function(data)
	if data and data.VIN then
		...
		if freeImpoundSpace and #(pedCoords - freeImpoundSpace.xyz) <= 50.0 then
			exports["pulsar-core"]:ServerCallback('Vehicles:RetrieveVehicleFromImpound', {
				VIN = data.VIN,
				...
				if success then
					...
					exports["pulsar-hud"]:Notification("error", 'Error Retrieving From Impound')
				end
			end)
		else
			exports["pulsar-hud"]:Notification("error", 'Impound Parking Lot Full')
		end
	end
end)
