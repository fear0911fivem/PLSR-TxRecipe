	...
	if entityData and entityData.entity and DoesEntityExist(entityData.entity) then
			...
			main = {
				...
				items = {
					...
					},
					{
						...
						description = (vehEnt.state.Mileage and exports['pulsar-core']:UtilsRound(vehEnt.state.Mileage, 2) or 0) ..
							" Miles",
						event = false,
					},
				},
		...
		for k, v in pairs(partNames) do
			if vehDamage and vehDamage[k] then
				...
				table.insert(menu.main.items, {
					label = v,
					description = exports['pulsar-core']:UtilsRound(vehDamage[k], 2) .. "%",
					event = false,
				})
...
AddEventHandler("Mechanic:Client:InstallMultipleRepairParts", function(data)
	exports["pulsar-core"]:ServerCallback("Mechanic:InstallMultipleRepairParts", data, function(success)
		if not success then
			exports["pulsar-hud"]:Notification("error", "Unable to Install Multiple Parts")
		end
	end)
end)

AddEventHandler("Mechanic:Client:RemovePerformanceUpgrade", function(data)
	exports["pulsar-core"]:ServerCallback("Mechanic:RemovePerformanceUpgrade", data, function(success)
		if not success then
			exports["pulsar-hud"]:Notification("error", "Unable to Remove Upgrade")
		end
	end)
end)
