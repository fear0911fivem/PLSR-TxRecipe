...
RegisterNetEvent("Robbery:Client:ThermiteFx", function(delay, netId)
	...
	if bombObj ~= 0 then
		...
		local effect1 = StartNetworkedParticleFxLoopedOnEntity("scr_heist_ornate_thermal_burn", bombObj, 0, 1.0, 0, 0, 0,
			0, 1.0, false, false, false)
		local effect2 = StartNetworkedParticleFxLoopedOnEntity("scr_heist_ornate_thermal_sparks", bombObj, 0, 1.0, 0, 0,
			0, 0, 1.0, false, false, false)
		local effect3 = StartNetworkedParticleFxLoopedOnEntity("scr_heist_ornate_thermal_glow", bombObj, 0, 1.0, 0, 0, 0,
			0, 1.0, false, false, false)
		local effect4 = StartNetworkedParticleFxLoopedOnEntity("sp_fbi_fire_trail_smoke", bombObj, 0, 1.0, 0, 0, 0, 0,
...
RegisterNetEvent("Robbery:Client:BombFx", function(loc)
	...
	StartNetworkedParticleFxNonLoopedAtCoord("ent_ray_paleto_gas_explosion", loc.x, loc.y, loc.z, 0.0, 0.0, 0.0, 2.5,
...
RegisterNetEvent("Robbery:Client:DetCordFx", function(loc)
	...
	UseParticleFxAssetNextCall("des_gas_station")
	SetParticleFxNonLoopedColour(1.0, 0.0, 0.0)
	StartNetworkedParticleFxNonLoopedAtCoord("ent_ray_paleto_gas_explosion", loc.x, loc.y, loc.z, 0.0, 0.0, 0.0, 0.15,
		false, false, false, false)
	AddExplosion(
		loc.x,
		loc.y,
...
function LaptopShit(loc, data, cb)
	...
	Wait(0)
	TaskPlayAnimAdvanced(ply, hackAnimDict, "hack_enter", vector3(loc.x, loc.y, loc.z), 0, 0, 0, 1.0, 0.0, 8300, 0, 0.3,
		false, false, false)
	Wait(0)
	SetEntityHeading(ply, loc.h)
	...
	DoKeymaster(data.config, data.data, function(isSuccess, data)
		p:resolve(isSuccess)
		exports["pulsar-core"]:DeleteObject(laptop)
		ClearPedTasksImmediately(ply)
	end)
...
function ThermiteShit(loc, data, cb)
	...
	CreateThread(function()
		...
		local bagscene = NetworkCreateSynchronisedScene(loc.x, loc.y, loc.z, rotx, roty, rotz + 0.0, 2, false, false,
			1065353216, 0, 1.3)
		...
		AttachEntityToEntity(bomba, ped, GetPedBoneIndex(ped, 28422), 0, 0, 0, 0, 0, 200.0, true, true, false, true, 1,
			true)
		Wait(3500)
		...
		DoMemory(data.passes, data.config, data.data, function(isSuccess, extra)
			if isSuccess then
				...
				TaskPlayAnim(ped, "anim@heists@ornate_bank@thermal_charge", "cover_eyes_intro", 8.0, 8.0, 1000, 36, 1, 0,
					0, 0)
				TaskPlayAnim(ped, "anim@heists@ornate_bank@thermal_charge", "cover_eyes_loop", 8.0, 8.0, 6000, 49, 1, 0,
					0, 0)
				Wait(2000)
				ClearPedTasks(ped)
...
function HackShit(data, cb)
	...
	AttachEntityToEntity(_mdtProp, ply, GetPedBoneIndex(ply, 60309), 0.02, -0.01, -0.03, 0.0, 0.0, -10.0, 1, 0, 0, 0, 2,
		1)
	...
	CreateThread(function()
		while doingShit do
			if not IsEntityPlayingAnim(ply, "amb@code_human_in_bus_passenger_idles@female@tablet@base", "base", 3) then
				TaskPlayAnim(ply, "amb@code_human_in_bus_passenger_idles@female@tablet@base", "base", 3.0, 3.0, -1, 49, 0,
					false, false, false)
			end
			Wait(250)
		end
	...
	DoIcons(data.config, data.data, function(isSuccess, data)
		...
		exports["pulsar-core"]:DeleteObject(_mdtProp)
		ClearPedTasksImmediately(ply)
	end)
