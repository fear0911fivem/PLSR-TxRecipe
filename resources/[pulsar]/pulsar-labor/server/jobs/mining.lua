...
local function GenerateNodes(source)
	...
	if not antiDouche[source] then
		...
		local repLevel = exports['pulsar-characters']:RepGetLevel(source, "Mining") or 0
...
AddEventHandler("Labor:Server:Startup", function()
	exports["pulsar-core"]:RegisterServerCallback("Mining:SellStone", function(source, data, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(source)
		...
		if (count or 0) > 0 then
			if exports.ox_inventory:Remove(char:GetData("SID"), 1, "crushedrock", count) then
				exports['pulsar-finance']:WalletModify(source, (3 * count))
				cb(true)
			else
				exports['pulsar-hud']:Notification(source, "error", "Unable To Remove Crushed Rock")
				cb(false)
			end
		else
			exports['pulsar-hud']:Notification(source, "error", "You Have No Crushed Rock")
			cb(false)
		end
	end)

	exports["pulsar-core"]:RegisterServerCallback("Mining:SellGem", function(source, data, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(source)
	...
	exports["pulsar-core"]:RegisterServerCallback("Mining:StartJob", function(source, data, cb)
		if _mining[_joiners[source]] ~= nil and _mining[_joiners[source]].state == 0 then
			...
			exports['pulsar-labor']:StartOffer(_joiners[source], _JOB, "Mine Ore Nodes", 15)
	...
	exports["pulsar-core"]:RegisterServerCallback("Mining:Server:MineNode", function(source, data, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(source)
		if char:GetData("TempJob") == _JOB and _joiners[source] ~= nil and _mining[_joiners[source]] ~= nil then
				...
				for i = #_mining[_joiners[source]].nodes, 1, -1 do
					...
					if node then
						...
						then
							...
							exports['pulsar-labor']:SendWorkgroupEvent(j,
								string.format("Mining:Client:%s:RemoveNode", j), node.location)
							if exports['pulsar-labor']:UpdateOffer(j, _JOB, 1, false) then
								antiDouche[j] = false
							elseif not _mining[j].nodes or #_mining[j].nodes == 0 then
								antiDouche[j] = false
								exports['pulsar-labor']:ManualFinishOffer(j, _JOB)
							else
						...
						if not _mining[j].nodes or #_mining[j].nodes == 0 then
							...
							exports['pulsar-hud']:Notification(source, "error", "Invalid Node")
						end
					end
				end
			else
			...
			exports['pulsar-hud']:Notification(source, "error", "Incorrect Job")
		end
	end)
end)
...
AddEventHandler("Mining:Server:OnDuty", function(joiner, members, isWorkgroup)
	...
	exports['pulsar-labor']:TaskOffer(joiner, _JOB, "Talk To The Foreman")
	if #members > 0 then
