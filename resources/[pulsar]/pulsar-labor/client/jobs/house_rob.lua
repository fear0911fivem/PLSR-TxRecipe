...
AddEventHandler("Labor:Client:Setup", function()
    ...
    exports['pulsar-pedinteraction']:Add("HouseRobbery", `csb_grove_str_dlr`, _queueLoc.coords, _queueLoc.heading, 25.0,
        {
            {
    ...
    exports["pulsar-core"]:RegisterClientCallback("HouseRobbery:Lockpick", function(data, cb)
        _lpStage = 0
        DoLockpick({ timer = 1.0, base = 5 }, data, cb)
        exports['pulsar-mdt']:EmergencyAlertsCreateIfReported(40.0, "bane", true)
    end)

    exports["pulsar-core"]:RegisterClientCallback("HouseRobbery:AdvLockpick", function(data, cb)
        _lpStage = 0
        DoLockpick({ timer = 0.75, base = 8 }, data, cb)
        exports['pulsar-mdt']:EmergencyAlertsCreateIfReported(40.0, "bane", true)
    end)

    exports['pulsar-hud']:InteractionRegisterMenu("house-robbery", "Enter House", "window-frame-open", function(data)
    ...
    exports['pulsar-hud']:InteractionRegisterMenu("house-robbery-exit", "Exit House", "door-open", function(data)
        exports['pulsar-hud']:InteractionHide()
        ExitHouse()
    end, function()
...
function DoAlarm(pId)
    ...
    CreateThread(function()
        while _threading and _working and (not _nodes.states.alarm.disabled and not _nodes.states.alarm.triggered) and LocalPlayer.state.inRobbedHouse do
            exports["pulsar-sounds"]:PlayOne("alarm_warn.ogg", 0.1)
            Wait(1000)
        end
        exports["pulsar-sounds"]:StopOne("alarm_warn.ogg")
        _threading = false
    end)
end

function ExitHouse()
    exports["pulsar-core"]:ServerCallback("HouseRobbery:Exit", {}, function(propId, intr)
        LocalPlayer.state.inRobbedHouse = false
        ...
        exports["pulsar-sounds"]:StopOne("alarm_warn.ogg")
        exports["pulsar-sounds"]:StopOne("house_alarm.ogg", 0.1)
        ...
        exports['pulsar-polyzone']:Remove("property-house-rob-zone")
    end)
end
...
function EnterHouse()
    if _p ~= nil and _working then
        exports["pulsar-core"]:ServerCallback("HouseRobbery:BreakIn", _p, function(r, f, exit)
            if r then
...
RegisterNetEvent("HouseRobbery:Client:OnDuty", function(joiner, time)
    ...
    eventHandlers["receive"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Receive", joiner),
        function(propertyId, data)
            _p = propertyId
            _state = 1
            ...
            if _blip then
                exports["pulsar-blips"]:Remove("HouseRobbery")
            end
            _blip = exports["pulsar-blips"]:Add("HouseRobbery", "Target House",
            ...
            CreateThread(function()
                ...
                exports["pulsar-core"]:ServerCallback("HouseRobbery:ArrivedNear", {})
            end)
        end)
    ...
    eventHandlers["near"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Near", joiner), function(data)
        while _working and _p ~= nil do
            if LocalPlayer.state.inRobbedHouse then
                ...
                if dist < 20 then
                    DrawMarker(1, data.x, data.y, data.z + 0.5, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 1.0, 250, 250, 250, 250,
    ...
    eventHandlers["lockpicked"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:Lockpicked", joiner),
        function(data)
            _nodes = data
            _state = 4
            exports["pulsar-hud"]:Notification("success", "House Has Been Lockpicked")
        end)
    ...
    eventHandlers["hacked"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:AlarmHacked", joiner),
        function(data)
    ...
    eventHandlers["alarm-trigger"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:AlarmTriggered", joiner),
        function(data)
            if _working and not _nodes.states.alarm.triggered and not _nodes.states.alarm.disabled then
                ...
                if LocalPlayer.state.inRobbedHouse then
                    exports["pulsar-sounds"]:LoopOne("house_alarm.ogg", 0.1)
                end
            end
        end)
            ...
            if data.id and not _nodes.searched[data.id] then
                exports['pulsar-hud']:ProgressWithTickEvent({
                    ...
                    if not cancelled then
                        exports['pulsar-status']:Add("PLAYER_STRESS", 3, false, true)
                        exports["pulsar-core"]:ServerCallback("HouseRobbery:Search", data.id)
                    end
                end)
            end
        end)
    ...
    eventHandlers["inner"] = RegisterNetEvent(string.format("HouseRobbery:Client:%s:InnerStuff", joiner), function(intr)
        ...
        if _nodes.chances.alarm then
            ...
            elseif _nodes.states.alarm.triggered then
                exports["pulsar-sounds"]:PlayOne("house_alarm.ogg", 0.1)
            end
        else
            exports.ox_target:addEntity(o, {
                {
        ...
        if intr.zone then
            exports['pulsar-polyzone']:CreateBox("property-house-rob-zone", intr.zone.center, intr.zone.length,
                intr.zone.width,
                intr.zone.options, {})
        end
...
AddEventHandler("HouseRobbery:Client:Enable", function()
    exports["pulsar-core"]:ServerCallback('HouseRobbery:Enable', {})
end)

AddEventHandler("HouseRobbery:Client:StartJob", function()
    exports["pulsar-core"]:ServerCallback('HouseRobbery:StartJob', _joiner, function(state)
        if not state then
...
AddEventHandler("HouseRobbery:Client:HackSuccess", function(data)
    ...
    if _scPass > 3 then
        _scPass = 1
        exports["pulsar-core"]:ServerCallback("HouseRobbery:HackAlarm", {
            propertyId = data.pId,
            state = true
        }, function()
            exports["pulsar-hud"]:Notification("success", "Alarm Bypassed")
        end)
    else
        Wait(1500)
        DoHack(data.pId)
...
function DoLockpick(diff, data, cb)
    exports['pulsar-games']:MinigamePlayRoundSkillbar((diff.timer + (0.2 * data.tier)), diff.base - stageComplete, {
        onSuccess = function()
            Wait(400)
...
function DoHack(pId)
    exports['pulsar-games']:MinigamePlaySequencer(5, 300, 7500 - (1000 * _scPass), 3 + _scPass, true, {
