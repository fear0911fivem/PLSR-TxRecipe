	...
	if business then
		if _bizPhones[business] and _bizPhones[business].call then
			...
			if caller ~= nil and _calls[caller] ~= nil then
				exports['pulsar-core']:LoggerTrace("Phone",
				...
				exports['pulsar-phone']:CallCreateRecord(_calls[caller].data)
				exports['pulsar-phone']:NotificationRemoveById(caller, "PHONE_CALL")
				exports["pulsar-voip"]:SetCall(caller, 0)
				_calls[caller] = nil
			else
			...
			if _bizPhones[business].call.handler then
				exports["pulsar-voip"]:SetCall(_bizPhones[business].call.handler, 0)
		...
		if _calls[source] ~= nil then
			...
			exports['pulsar-phone']:CallCreateRecord(_calls[source].data)
			...
			exports['pulsar-phone']:NotificationRemoveById(source, "PHONE_CALL")
			exports["pulsar-voip"]:SetCall(source, 0)

			if _calls[source].isBiz then
				if _bizPhones[_calls[source].isBiz].call and _bizPhones[_calls[source].isBiz].call.handler then
					exports["pulsar-voip"]:SetCall(_bizPhones[_calls[source].isBiz].call.handler, 0)
				...
				if _calls[source].target ~= nil and _calls[_calls[source].target] ~= nil then
					...
					exports['pulsar-phone']:CallCreateRecord(_calls[_calls[source].target].data)
					exports['pulsar-phone']:NotificationRemoveById(_calls[source].target, "PHONE_CALL")
					exports["pulsar-voip"]:SetCall(_calls[source].target, 0)
...
AddEventHandler("Characters:Server:PlayerLoggedOut", function(source, cData)
	if _calls[source] ~= nil then
		exports['pulsar-phone']:CallEnd(source)
	end

	if _bizCallHandlers[source] ~= nil then
		exports['pulsar-phone']:CallEnd(-1, _bizCallHandlers[source])
	end
end)
...
AddEventHandler("Phone:Server:RegisterCallbacks", function()
	exports["pulsar-core"]:RegisterServerCallback("Phone:Phone:CreateCall", function(src, data, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(src)
		if _calls[src] == nil and char:GetData("Phone") ~= data.number then
			local callingContact = exports['pulsar-phone']:ContactsIsContact(char:GetData("SID"), data.number)
			...
			exports['pulsar-phone']:NotificationAddWithId(src, "PHONE_CALL", "Dialing", callingStr, os.time(), -1,
				"phone", {
			...
			if isBusiness and _bizPhones[isBusiness] then
				...
				if bizData.call then -- Already Busy
					exports['pulsar-phone']:CallCreateRecord(callerData)
					...
					cb(false)
					exports['pulsar-phone']:CallEnd(src)
				else
			...
			if target ~= nil and exports.ox_inventory:GetItemCount(target:GetData("Source"), 'phone') > 0 then
				if _calls[target:GetData("Source")] == nil then
					...
					local destContact = exports['pulsar-phone']:ContactsIsContact(target:GetData("SID"),
						char:GetData("Phone"))
					local destStr = char:GetData("Phone")
					if destContact then
					...
					exports['pulsar-phone']:CallCreateRecord(recipData)
				...
				exports['pulsar-phone']:CallCreateRecord({
					owner = data.number,
					number = char:GetData("Phone"),
					time = os.time(),
					duration = -1,
	...
	exports["pulsar-core"]:RegisterServerCallback("Phone:Phone:AcceptCall", function(src, data, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(src)
		...
		if _calls[src] ~= nil then
			if _calls[src].isBiz and _bizPhones[_calls[src].isBiz].call then
				...
				exports["pulsar-voip"]:SetCall(src, cid)
				exports["pulsar-voip"]:SetCall(bizCallerSource, cid)
			elseif _calls[src].target ~= nil and _calls[_calls[src].target] ~= nil then
				...
				exports["pulsar-voip"]:SetCall(src, cid)
				exports["pulsar-voip"]:SetCall(_calls[src].target, cid)
			end
		else
			exports['pulsar-core']:LoggerTrace(
				"Phone",
	...
	exports["pulsar-core"]:RegisterServerCallback("Phone:Phone:EndCall", function(src, data, cb)
		exports['pulsar-phone']:CallEnd(src)
	end)

	exports["pulsar-core"]:RegisterServerCallback("Phone:Phone:ReadCalls", function(src, data, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(src)
		if char ~= nil then
			exports['pulsar-phone']:CallRead(char:GetData("Phone"))
		end
	end)

	exports["pulsar-core"]:RegisterServerCallback("Phone:MuteBiz", function(source, id, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(source)
		if char ~= nil and id and _bizPhones[id] and exports['pulsar-jobs']:HasJob(source, _bizPhones[id].job) then
	...
	exports["pulsar-core"]:RegisterServerCallback("Phone:DeclineBizCall", function(source, id, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(source)
		if char ~= nil and id and _bizPhones[id] and _bizPhones[id].call and exports['pulsar-jobs']:HasJob(source, _bizPhones[id].job) then
			exports['pulsar-phone']:CallEnd(-1, id)
		else
			cb(false)
		end
	end)

	exports["pulsar-core"]:RegisterServerCallback("Phone:AcceptBizCall", function(source, id, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(source)
		if char ~= nil and not _calls[source] and id and _bizPhones[id] and _bizPhones[id].call and not _bizPhones[id].call.outgoing and exports['pulsar-jobs']:HasJob(source, _bizPhones[id].job) then
			if not _bizPhones[id].call.handler then
				...
				exports["pulsar-voip"]:SetCall(source, cid)
				exports["pulsar-voip"]:SetCall(targetSource, cid)
	...
	exports["pulsar-core"]:RegisterServerCallback("Phone:MakeBizCall", function(source, data, cb)
		local char = exports['pulsar-characters']:FetchCharacterSource(source)
