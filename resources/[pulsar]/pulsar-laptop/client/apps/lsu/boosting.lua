...
AddEventHandler("Characters:Client:Spawn", function()
    Wait(1000)
    exports['pulsar-hud']:RegisterStatus("boosting-timer", 0, 100, "timer", "#892020", false, false, {
        hideZero = true,
    })
end)
...
RegisterNetEvent("Laptop:Client:LSUnderground:Boosting:Start", function(data)
    ...
    if _boosting then
        ...
        exports['pulsar-polyzone']:CreateCircle("boosting-dropoff", _boosting.dropOff, 30.0, {})
    end
end)
...
RegisterNetEvent("Laptop:Client:LSUnderground:Boosting:UpdateState", function(state, data)
    ...
    if state == 1 then
        exports["pulsar-blips"]:Remove("boosting-contract")
    elseif state == 2 then
        if data then
    ...
    elseif state == 3 and _boosting.dropOff then
        exports["pulsar-hud"]:Notification("remove", nil, nil, nil, nil, "boosting-trackers")
        exports["pulsar-blips"]:Add(
            "boosting-contract",
...
RegisterNetEvent("Laptop:Client:LSUnderground:Boosting:TrackerNotificationUpdate", function(notif)
    exports["pulsar-hud"]:Notification("info", notif, -1, nil, nil, "boosting-trackers")
end)
...
AddEventHandler("Laptop:Client:LSUnderground:Boosting:AttemptExterior", function(veh)
    if _boosting and NetworkDoesEntityExistWithNetworkId(_boosting.vehicleNet) then
        if veh == NetToVeh(_boosting.vehicleNet) then
            exports["pulsar-core"]:ServerCallback("Laptop:LSUnderground:Boosting:Exterior", {},
                function(numPeds, makeDifficult, noAlert)
                    if numPeds then
                        ...
                        CreateThread(function()
                            for i = 1, numPeds do
                                ...
                                local w = exports['pulsar-core']:UtilsWeightedRandom(availableWeapons)
                    ...
                    if not noAlert then
                        ...
                        TriggerServerEvent("EmergencyAlerts:Server:DoPredefined",
                            isTracked and "boostingTracked" or "boosting", {
                                icon = "car",
...
AddEventHandler("Laptop:Client:LSUnderground:Boosting:SuccessIgnition", function(veh)
    if _boosting and NetworkDoesEntityExistWithNetworkId(_boosting.vehicleNet) then
        if veh == NetToVeh(_boosting.vehicleNet) then
            ...
            exports["pulsar-core"]:ServerCallback("Laptop:LSUnderground:Boosting:Ignition", {
...
function RegisterBoostingCallbacks()
    exports["pulsar-core"]:RegisterClientCallback("Laptop:LSUnderground:Boosting:TrackerHacker", function(data, cb)
        ...
        if _boosting and (not trackerTimer or GetGameTimer() >= trackerTimer) and hackData and NetworkDoesEntityExistWithNetworkId(_boosting.vehicleNet) then
            ...
            if GetEntitySpeed(veh) >= 15.0 and GetPedInVehicleSeat(veh, 0) == LocalPlayer.state.ped then
                exports['pulsar-games']:MinigamePlayPattern(
                    ...
                    hackData.charSize,
                    exports['pulsar-core']:UtilsWeightedRandom(hackData.charSet), {
                        onSuccess = function()
...
AddEventHandler("Polyzone:Exit", function(id)
    if id == "boosting-dropoff" and inZone then
        ...
        if _boosting and _boosting.state == 4 then
            exports["pulsar-core"]:ServerCallback("Laptop:LSUnderground:Boosting:LeftArea", {})
        end
    end
end)
...
AddEventHandler("Vehicles:Client:ExitVehicle", function(veh)
    ...
    then
        exports["pulsar-core"]:ServerCallback("Laptop:LSUnderground:Boosting:DropOff", {})
    end
end)
...
function CleanUpBoosting()
    if _boosting then
        ...
        exports['pulsar-polyzone']:Remove("boosting-dropoff")
        exports["pulsar-blips"]:Remove("boosting-contract")
...
RegisterNetEvent("Laptop:Client:Teams:Set", function(teamData)
    if not teamData and wasBoosting then
        exports['pulsar-laptop']:Notification("remove", "BOOSTING_CONTRACT")
