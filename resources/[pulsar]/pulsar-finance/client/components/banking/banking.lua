...
function RunBankingStartup()
	...
	for k, v in pairs(_bankBranches) do
		if v.interactZone then
			exports['pulsar-polyzone']:CreateBox("banking-" .. k, v.interactZone.center, v.interactZone.length,
...
AddEventHandler("Characters:Client:Spawn", function()
	for k, v in pairs(_bankBranches) do
		if v.blip then
			if v.special then
				exports["pulsar-blips"]:Add("bank_" .. k, "Pacific Bank", v.blip, 272, 15, 0.8)
			else
				exports["pulsar-blips"]:Add("bank_" .. k, "Bank", v.blip, 272, 69, 0.6)
			end
		end
	end
end)
...
AddEventHandler("Polyzone:Enter", function(id, point, insideZone, data)
	if data.bank_branch then
		...
		if not GlobalState[string.format("Fleeca:Disable:%s", data.bank_branch)] then
			exports['pulsar-hud']:ActionShow("banking", "{keybind}primary_action{/keybind} Open Bank")
		else
			exports['pulsar-hud']:ActionShow("banking", "Bank Unavailable")
		end
...
AddEventHandler("Polyzone:Exit", function(id, point, insideZone, data)
	if withinBranchZone and data and data.bank_branch then
		...
		if showingAction then
			exports['pulsar-hud']:ActionHide("banking")
			showingAction = false
		end
	end
end)
