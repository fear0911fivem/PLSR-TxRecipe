...
function RunBankingStartup()
	...
	if info and info.total then
		exports['pulsar-core']:LoggerInfo("Banking",
	...
	if stateAccount and stateAccount.balance then
		...
		exports['pulsar-core']:LoggerError("Banking", "Failed to load State Government Account")
	end
	...
	if d.affectedRows > 0 then
		exports['pulsar-core']:LoggerInfo("Banking", "Cleared ^2" .. d.affectedRows .. "^7" .. " Old Bank Transactions")
	end
end
...
AddEventHandler("Finance:Server:Startup", function()
	exports['pulsar-core']:MiddlewareAdd("Characters:Spawning", function(source)
		...
		if char and not char:GetData("BankAccount") then
			...
			local bankAccountData = exports['pulsar-finance']:AccountsCreatePersonal(stateId)
...
function CreateOrganizationBankAccounts()
	...
	local allJobs = exports['pulsar-jobs']:GetAll()
	if not allJobs then
		return
	end
	...
	if created > 0 then
		exports['pulsar-core']:LoggerTrace("Banking", "Created ^2" .. created .. "^7 Default Organization Accounts")
	end
end
