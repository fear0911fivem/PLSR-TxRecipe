...
exports('ReportsSearch', function(term, rType, page, perPage, isAttorney, evidence)
    local qry =
...
exports('ReportsSearchEvidence', function(term)

end)

exports('ReportsView', function(id)
...
exports('ReportsCreate', function(data)
...
exports('ReportsUpdate', function(id, char, report)
    local transaction = {
        {
...
exports('ReportsDelete', function(id)
    MySQL.query.await("DELETE FROM mdt_reports WHERE id = ?", { id })
    return true
end)

AddEventHandler("MDT:Server:RegisterCallbacks", function()
    exports["pulsar-core"]:RegisterServerCallback("MDT:Search:report", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if CheckMDTPermissions(source, false) or (char:GetData("Attorney") and data.isAttorney) then
            cb(exports['pulsar-mdt']:ReportsSearch(data.term, data.reportType, data.page, data.perPage, data.isAttorney,
                data.evidence))
        else
            cb(false)
        end
    ...
    exports["pulsar-core"]:RegisterServerCallback("MDT:Search:report-evidence", function(source, data, cb)
        if CheckMDTPermissions(source, false) then
            cb(exports['pulsar-mdt']:ReportsSearchEvidence(data.term))
        else
            cb(false)
        end
    end)

    exports["pulsar-core"]:RegisterServerCallback("MDT:Create:report", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if CheckMDTPermissions(source, false) then
            data.doc.author = {
            ...
            cb(exports['pulsar-mdt']:ReportsCreate(data.doc))
        else
            cb(false)
        end
    end)

    exports["pulsar-core"]:RegisterServerCallback("MDT:Update:report", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if char and CheckMDTPermissions(source, false) then
            cb(exports['pulsar-mdt']:ReportsUpdate(data.id, char, data.report))
        else
            cb(false)
        end
    end)

    exports["pulsar-core"]:RegisterServerCallback("MDT:Delete:report", function(source, data, cb)
        if CheckMDTPermissions(source, true) then
            cb(exports['pulsar-mdt']:ReportsDelete(data.id))
        else
            cb(false)
        end
    end)

    exports["pulsar-core"]:RegisterServerCallback("MDT:View:report", function(source, data, cb)
        local char = exports['pulsar-characters']:FetchCharacterSource(source)
        if CheckMDTPermissions(source, false) or char:GetData("Attorney") then
            cb(exports['pulsar-mdt']:ReportsView(data))
        else
            cb(false)
        end
    end)
